...
aggregate_last_x_games <- function(game_date, player, columns_to_summarise, n, sum_fun){
  ...
  filtered_data <- train2 %>% 
    filter(first_last == player, date < game_date) %>%
    top_n(n, date)
  vars = c("first_last", columns_to_summarise)
...
create_aggregae_last_x_games <- function(n, sum_fun){
  columns_to_summarise <- colnames(train2)[sapply(train2,is.numeric)]
  columns_to_summarise <- columns_to_summarise[!grepl("gid|pos|gtime_et_", columns_to_summarise)]
  # n <- 1
  # sum_fun <- "mean"
  game_date <- train2$date[14444]
  player <- train2$first_last[14444]
  template <- aggregate_last_x_games(game_date, player, columns_to_summarise, n, sum_fun)[1,]
  template <- template[-1,]
  
  for(row in 1:nrow(train2)){
    temp <- aggregate_last_x_games(train2$date[row], train2$first_last[row], columns_to_summarise, n, sum_fun)
    if(nrow(temp) == 0){
      temp <- template[1,]
      temp$first_last <- train2$first_last[row]
      temp$date <- train2$date[row] 
      }
...
vegas_odds <- fread('vegas_odds_swapped.csv')
...
asd <- with(train2, mapply(function(x, y) paste0(x,",",y), team, opp))
train2$asd <- asd
...
train2 <- cbind(train2, past_5 %>% select(-first_last, -date))
train2 <- cbind(train2, past_3 %>% select(-first_last, -date))
train2 <- cbind(train2, past_1 %>% select(-first_last, -date))

f <- ~ first_last + fd_sal + team + opp + h_a + time + last_1_games_mean_minutes + last_3_games_mean_minutes + last_5_games_mean_minutes + overunder + margin + split(pool, delim = ",") + split(pool_day, delim = ",") - 1 +
  split(asd, delim = ",")
...
set.seed(381)
m1 <- xgb.train(data = dmodel, param, nrounds = 5000,
                watchlist = watch
...
result %>% select(first_last, date, multiplier, pred, fd_sal, fdp) %>% mutate(proj_pts = fd_sal/1000 * pred) %>% filter(date == "2017-01-08") %>% View
...
write_csv(vegas, "vegas_0120.csv")
vegas <- fread('vegas_0120.csv', stringsAsFactors = F)
today <- fread("0120.csv") # Download live from fanduel!!
...
today$team <- gsub("^sa[^c]", "sas", today$team)
today$opp  <- gsub("^sa[^c]", "sas", today$opp)
...
view2 <- today_results %>% select(first_last, fd_pos, fd_sal, prediction) %>% mutate(proj_pts = fd_sal*prediction/1000) %>% arrange(fd_pos, -prediction)
